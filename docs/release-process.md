...
```bash
git tag v0.3.1 # regular release
...
**Important**: The tag must start with `v` (e.g., `v0.3.1`, not `0.3.1`).
...
Once you publish the release (or push a tag), the `publish-release` workflow automatically:
...
### 4. Alternative Trigger Methods

The workflow can also be triggered by:

- **Tag push**: Simply pushing a tag (e.g., `git push origin v0.3.1`) without creating a GitHub release
- **Manual dispatch**: Using the GitHub Actions UI to manually trigger the workflow with a specific tag (useful for testing)
